...
SWEP.PrintName = "Oi Gun"
SWEP.Category = "AFGMod's Weapons"
...
SWEP.Secondary.Ammo = "rb655_oi"

game.AddAmmoType( { name = "rb655_oi" } )
if ( CLIENT ) then language.Add( "rb655_oi_ammo", "Annoying Ammo" ) end

/* --------------------------------- TTT ---------------------------------- */

SWEP.EquipMenuData = {
	type = "item_weapon",
	desc = [[The oi Gun. Provides:
	* Blown cover
	* Annoying sounds
...
function SWEP:Reload()
	...
	if ( SERVER ) then
		...
		local ent = ents.Create( "ent_oi_bomb" )
		if ( IsValid( ent ) ) then
...
function SWEP:DoImpactEffect( trace, damageType )
	...
	util.Effect( "io_bounce", effectdata )
...
killicon.Add( "weapon_oigun", "oi/killicon", color_white )

SWEP.WepSelectIcon = Material( "oi/selection.png" )
